...
int PTM_LoadTextureFile(const char* filename, PTMHead* ptmh, int flags, int effects)
{
	...
	if (!(flags & PTH_NOCOMPRESS) && glinfo.texcompr && glusetexcache && glusetexcompr && bglCompressedTexImage2DARB) {
...
static void ptm_uploadtexture(PTMHead * ptm, unsigned short flags, PTTexture * tex, PTCacheTile * tdef)
{
	...
	if (!(flags & PTH_NOCOMPRESS) && glinfo.texcompr && glusetexcompr && bglCompressedTexImage2DARB) {
