...
static void ceilscan(long x1, long x2, long sectnum)
{
	...
	setgotpic(globalpicnum);
	DO_TILE_ANIM(globalpicnum,(short)sectnum);
...
#if MEGAWANG
    float k = (xdim/(float)ydim)/1.33333333f;
#else
	float xfov;
...
#if MEGAWANG
    if (tallscreen) {
        windowy2 += 16; // 1280x1024 hack
    }
    setaspect(65536*xfov, divscale16(ydim*320, xdim*200/k));
#else
    new_aspect = tallscreen ? divscale16(ydim*320, xdim*200) : pixelaspect;
	setaspect((long)(65536.f * xfov), new_aspect);
#endif
