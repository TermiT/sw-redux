...
InitGame(VOID)
	...
	*/
...
#if MEGAWANG
        if (!loaddefinitionsfile("sw-redux.def")) {
            megawangdef = TRUE;
            initprintf("Mega wang definitions file loaded.\n");
        }
#endif
    ...
    {
        extern long ScreenMode, ScreenWidth, ScreenHeight, ScreenBPP;
//        play_mpeg_video("video/3drLogo.mpeg");
...
#if MEGAWANG
            if(megawangdef) {
                rotatesprite(320<<15, 200<<15, RS_SCALE, 0, MEGAWANG_TITLE_PIC, 0, 0, 2+8+64, 0, 0, xdim-1, ydim-1);
            } else {
...
#if MEGAWANG
            handleevents();
            if (KEY_PRESSED(KEYSC_ESC) || KEY_PRESSED(KEYSC_SPACE) || KEY_PRESSED(KEYSC_ENTER))
                break;
#else
...
IntroAnimLevel(VOID)
    {
      ...
      playanm(0);
#else
        play_vpx_video("video/wang.ivf", play_intro_sounds);
//        playanm(0);
#endif
    }
...
#if MEGAWANG
        if (!recordedLevelEnd && !UserMapName[0]) {
            recordLevelEnd(PlayingLevel, Skill, Player->Kills, TotalKillable, Player->SecretsFound, LevelSecrets, LevelInfo[PlayingLevel].BestTime, second_tics);
            recordedLevelEnd = TRUE;
        }
#endif
...
#if MEGAWANG
{1, "/addon#",             5,      "-addon[addon id]",     "Use to load official expansions"},
#endif
...
#if MEGAWANG
            if (Bstrncasecmp(arg, "addon", 5) == 0)
            {
                if (strlen(arg) > 5)
                {
                    swSetAddon(atoi(&arg[5]));
                }
            }
            
            if (Bstrncasecmp(arg, "noweaponhack", 13) == 0) {
                weaponhack = FALSE;
            }
...
#define SET_LOC_KEY(loc, sync_num, key_test) SET(loc, ((!!(key_test)) << (sync_num)))
...
#if MEGAWANG
        if (dimensionmode != 2 && dimensionmode != 5 && dimensionmode != 6) {
            if (BUTTON(gamefunc_Shrink_Screen) && !BUTTONHELD(gamefunc_Shrink_Screen)) {
                if (gs.BorderNum == BORDER_BAR)
                     SetBorder(pp, BORDER_MINI_BAR);
                else SetBorder(pp, BORDER_NONE);
                SetRedrawScreen(pp);
            }
            
            if (BUTTON(gamefunc_Enlarge_Screen) && !BUTTONHELD(gamefunc_Enlarge_Screen)) {
                if (gs.BorderNum == BORDER_NONE)
                    SetBorder(pp, BORDER_MINI_BAR);
                else if (gs.BorderNum == BORDER_MINI_BAR)
                    SetBorder(pp, BORDER_BAR);
                SetRedrawScreen(pp);
            }
        }
        
        if (BUTTON(gamefunc_Alt_Fire) && !BUTTONHELD(gamefunc_Alt_Fire)) {
            USERp u = User[pp->PlayerSprite];
            short this_weapon = u->WeaponNum + 1;
            SET(loc->bits, this_weapon);
        }
#endif
    ...
    if (BUTTON(gamefunc_Strafe) && !pp->sop)
        ...
        if (BUTTON(gamefunc_Turn_Left)
#if MEGAWANG
            || (BUTTON(gamefunc_Strafe_Left) && TEST(pp->Flags2, PF2_OPERATE))
#endif
            ...
            if (turnheldtime >= TURBOTURNTIME)
                ...
                if (BUTTON(gamefunc_Turn_Right)
#if MEGAWANG
      || (BUTTON(gamefunc_Strafe_Right) && TEST(pp->Flags2, PF2_OPERATE))
#endif
                    )
            {
            turnheldtime += synctics;
