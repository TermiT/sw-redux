...
#if MEGAWANG
char localsetupfilename[64]={LOCALSETUPFILENAME};
#endif
...
void CONFIG_SetupMouse( void ) {
    ...
    SCRIPT_GetNumber(scripthandle, "Controls", "MouseSensitivity", &value);
    dnSetMouseSensitivity(value);
    gs.MouseSpeed = value;
...
#if MEGAWANG
   SCRIPT_PutBoolean( scripthandle, "Screen Setup", "VerticalSync", vsync);
   SCRIPT_PutNumber( scripthandle, "Screen Setup", "MaxFPS", max_fps, FALSE, FALSE);
   dummy = (int32)(xfov*90.0f);
...
#if MEGAWANG
   SCRIPT_PutBoolean( scripthandle, "Options", "UseDarts", useDarts);
#endif
   ...
   WriteGameSetup(scripthandle);
       ...
       for (dummy = 0; dummy<NUMGAMEFUNCTIONS; dummy++) {
           SCRIPT_PutDoubleString( scripthandle, "KeyDefinitions", CONFIG_FunctionNumToName(dummy),
                                  (char*)dnGetKeyName(dnGetFunctionBinding(dummy, 0)),
                                  (char*)dnGetKeyName(dnGetFunctionBinding(dummy, 1)));
       }
#else
   for (dummy=0;dummy<NUMGAMEFUNCTIONS;dummy++) {
       SCRIPT_PutDoubleString( scripthandle, "KeyDefinitions", CONFIG_FunctionNumToName(dummy),
