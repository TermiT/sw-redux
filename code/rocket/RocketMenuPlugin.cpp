...
//	float rail_x, rail_y, rail_length;
	float m_min, m_max, value, step;

	RangeData():menu_item(NULL),hdr(NULL),ftr(NULL),range(NULL),
//        rail(NULL),grip(NULL),
		m_min(0.0f),m_max(1.0f),value(0.0f),step(0.1f)
	{}
	virtual ~RangeData() {};
};
...
void RocketMenuPlugin::SetupRange(Rocket::Core::Element *menu_item, Rocket::Core::Element *range_element) {
    ...
    if (range_element->HasAttribute("min")) {
		data->m_min = range_element->GetAttribute("min")->Get<float>();
	}
	if (range_element->HasAttribute("max")) {
		data->m_max = range_element->GetAttribute("max")->Get<float>();
	}
    ...
    input_attrs.Set("value", (data->value - data->m_min)/(data->m_max - data->m_min));
...
static
Rocket::Core::Element* findMenuItem(Rocket::Core::Element *e) {
    while (e != NULL && !e->GetParentNode()->IsClassSet("game-menu")) {
        e = e->GetParentNode();
    }
    return e;
}
...
//            Rocket::Core::Element *menu_item = element->GetParentNode()->GetParentNode();
            Rocket::Core::Element *menu_item = findMenuItem(element);
...
//            Rocket::Core::Element *menu_item = element->GetParentNode()->GetParentNode();
            ...
            float new_value = data->m_min + v*(data->m_max - data->m_min);
	...
	if (doc_data != NULL) {
		if (doc_data->active_item != e && !e->IsClassSet("disabled")) {
			if (doc_data->active_item != NULL) {
                doc_data->active_item->SetClass("highlighted", false);
...
//        printf("step: %s\n", tmp.CString());
        data->value = clamp(value, data->m_min, data->m_max);
        
        data->range->SetAttribute("value", (data->value - data->m_min)/(data->m_max - data->m_min));
#if 0
		float position = data->rail_x + data->value/(data->m_max-data->m_min)*data->rail_length;
